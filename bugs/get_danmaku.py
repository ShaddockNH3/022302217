...
def fetch_danmaku_xml(cid):
    ...
    try:
        resp = requests.get(DANMAKU_API_URL, params=params, headers=HEADERS, timeout=10)
        ...
        if xml_content:
            try:
                ...
                danmaku_list = []
                if root.tag != 'i': return None
                for d_element in root.findall('d'):
                    danmaku_list.append({'content': d_element.text, 'attributes': d_element.get('p')})
                return danmaku_list
...
if __name__ == "__main__":
    ...
    if os.path.exists(output_filename):
        with open(output_filename, 'r', encoding='utf-8') as f:
            try: all_videos_danmaku = json.load(f)
            except: pass
    if not os.path.exists(cid_list_file): exit(f"错误: 找不到输入文件 '{cid_list_file}'")
    with open(cid_list_file, 'r', encoding='utf-8') as f: cid_data = json.load(f)
    ...
    while i < len(video_keys):
        ...
        if danmaku_list is not None:
            ...
            all_videos_danmaku[bv] = {'cid': cid, 'danmaku_count': len(danmaku_list), 'danmaku_list': danmaku_list}
        else:
            print(f"  > 失败或无弹幕！暂时跳过 {bv}。")
            all_videos_danmaku[bv] = {'cid': cid, 'status': 'fetch_failed_or_no_danmaku'}

        try:
            with open(output_filename, 'w', encoding='utf-8') as f:
                json.dump(all_videos_danmaku, f, ensure_ascii=False, indent=2)
            print(f"  > 进度已保存 (当前进度: {len(all_videos_danmaku)}/{len(cid_data)} 条记录)")
