...
def process_danmaku_data(input_filename='json/LLM_danmaku_realtime_save.json'):
    ...
    for bv, video_data in all_data.items():
        if video_data.get('danmaku_list') and video_data.get('status') != 'fetch_failed_or_no_danmaku':
...
def write_to_excel(mode_totals, mode_content_freq, mode_word_freq, output_filename='xlsx/LLM_danmaku_statistics.xlsx'):
    """将统计数据写入Excel文件"""
    if not mode_totals:
    ...
    try:
        with pd.ExcelWriter(output_filename, engine='openpyxl') as writer:
            ...
            df_totals = pd.DataFrame(totals_data_list).sort_values(by='总数量', ascending=False)
            ...
            df_top_sentences.to_excel(writer, sheet_name='热门弹幕(整句)Top8', index=False)
            ...
            df_top_words.to_excel(writer, sheet_name='热门词频(分词)Top8', index=False)
...
if __name__ == "__main__":
    print("程序开始运行...")
    totals, freq_sentence, freq_word = process_danmaku_data(input_filename='json/LLM_danmaku_realtime_save.json')
    
    if totals:
        write_to_excel(totals, freq_sentence, freq_word, output_filename='xlsx/LLM_danmaku_statistics.xlsx')
    else:
        print("没有可以分析的数据。")
